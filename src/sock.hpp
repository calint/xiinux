...
class sock final {
  public:
    ...
    inline ~sock() {
        ...
        // not necessary since epoll removes the entry when the file descriptor
        // is closed. (man epoll -> see questions and answers)
        stats.errors++;
  ...
  private:
    inline auto do_after_headers() -> void {
        ...
        const std::string_view& path = reqline_.path_.at(0) == '/'
                                           ? reqline_.path_.substr(1)
                                           : reqline_.path_;
    ...
    auto do_serve_widget(widget_factory_func_ptr factory) -> void {
        ...
        if (!widget_) {
            ...
            session_->put_widget(std::string{reqline_.path_},
                                 std::move(up_wdgt));
        }
        ...
        if (session_id_.empty()) {
            ...
            if (gmtime_r(&timer, &tm_info) == nullptr) {
                throw client_exception{
                    "sock:retrieve_or_create_session:gmtime_r"};
            }
    ...
    inline auto do_serve_file(reply& x, const std::string_view& path) -> void {
        ...
        const size_t lastmod_len =
            strftime(lastmod.data(), lastmod.size(), "%a, %d %b %y %H:%M:%S %Z",
                     &tm_info);
        if (lastmod_len == 0) {
        ...
        if (suffix_ix != std::string_view::npos) {
            const std::string_view& suffix =
                path_resolved.substr(suffix_ix + 1);
            if (suffix == "js") {
    ...
    class content {
      ...
      public:
        ...
        inline auto init_for_receive(const std::string_view& content_length_str)
            ...
            if (!buf_) {
                buf_ = std::make_unique<
                    std::array<char, conf::sock_content_buf_size>>();
            }
        }
        ...
        inline auto receive_from(int fd_in) -> ssize_t {
            stats.reads++;
            const ssize_t n =
                recv(fd_in, buf_.get(), conf::sock_content_buf_size, 0);
            if (n == -1) { // error
                return n;
            }
