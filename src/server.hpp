...
class server final {
        ...
        while (true) {
            ...
            for (unsigned i = 0; i < unsigned(n); i++) {
                ...
                if (ev.data.ptr == &server_fd) {
                    ...
                    const int client_fd =
                        accept4(server_fd, client_sock_addr, &client_addr_len,
                                SOCK_NONBLOCK);
                    if (client_fd == -1) {
                        perror("accept");
                        continue;
                    ...
                    if (option_benchmark_mode) {
                        ...
                        option = 0;
                        if (setsockopt(client_fd, IPPROTO_TCP, TCP_CORK,
                                       &option, sizeof(option))) {
  ...
  private:
    ...
    inline static void print_client_exception(const sock* client,
        ...
        // output
        printf("!!! exception %s  %s  session=[%s]  path=[%s]  query=[%s]  "
               "msg=[%s]\n",
               time_str_buf.data(), ip_addr_to_str(ip_addr_str, &addr),
               ses_id.empty() ? "n/a" : ses_id.c_str(),
               client->get_path().data(), client->get_query().data(), msg);
    }
